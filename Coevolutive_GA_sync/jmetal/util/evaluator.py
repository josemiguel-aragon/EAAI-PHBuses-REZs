...
class CoevolvedEvaluator(Generic[S], ABC):
    ...
    @staticmethod
    def evaluate_solution(solution: S, problem: Problem, problem2_individual: S) -> None:
        problem.evaluate(solution, problem2_individual)
...
class CoevolvedSequentialEvaluator(Evaluator[S]):

    def evaluate(self, solution_list: List[S], problem: Problem, problem2_individual: S) -> List[S]:
        for solution in solution_list:
            CoevolvedEvaluator.evaluate_solution(solution, problem, problem2_individual)
