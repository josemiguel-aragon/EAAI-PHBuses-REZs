...
def get_final_front():
    ...
    all_fronts = []
    for exec_i in range(1,30+1):
        chdir(f'Coevolutive_GA_{exec_i}/')
        ...
        for process_id in tqdm(range(69+1)):
            ...
            green_kms = []
            emissions = []
            with open(f'{folder}FUN.998') as file:
            ...
            solutions = []
            with open(f'{folder}VAR.998') as file:
                for line in file.readlines():
            ...
            for index,_ in enumerate(solutions):
                ...
                for j,partner in enumerate(sol.partners):
                    route = read_route(config_route_paths[j])
                    bus  = Bus(1, route)
    ...
    with open('./results_all.data', 'w') as file:
        file.write("Final Pareto Front:")
        for sol in final_front:
            file.write("\n\tSolution:")
            for index,_ in enumerate(route_labeling):
