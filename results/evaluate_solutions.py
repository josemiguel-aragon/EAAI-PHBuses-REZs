...
if __name__ == '__main__':
    ...
    for sol in solutions:
        ...
        for index,_ in enumerate(sol):
            ...
            green_kms, emissions, green_zone_emissions = evaluate(sol[index], route, bus)

            total_green_kms += green_kms
            total_emissions += emissions
            total_green_zone_emissions += green_zone_emissions

        all_g.append(total_green_kms)
        all_e.append(total_emissions)
        all_g_z.append(total_green_zone_emissions)
    with open('results_emissions_greenkms.data', 'w') as file:
        file.write('Final Pareto Front:')
        for j,sol in enumerate(solutions):
            file.write('\n\tSolution:')
            for index,_ in enumerate(sol):
                file.write(f'\n\t\t{config_route_paths[index]}')
                file.write(f'\n\t\tVariables: {sol[index]}')
            file.write(f"\n\tGreen_kms: {all_g[j]}")
            file.write(f"\n\tEmissions: {all_e[j]}")
            file.write(f"\n\tLow-Emissions Zone: {all_g_z[j]}")
